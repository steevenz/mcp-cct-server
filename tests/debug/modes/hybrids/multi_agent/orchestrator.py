...
from src.core.constants import DEFAULT_MODEL_TIER
...
class MultiAgentFusionEngine(BaseCognitiveEngine):
    ...
    def execute(self, session_id: str, input_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        validated_input = self._validate_payload(input_payload, MultiAgentFusionInput)
        ...
        for persona in validated_input.personas:
            ...
            self._score_and_save(session_id, p_thought, history, model_id=session.model_id)

            persona_nodes.append(p_thought)
            # Link to parent (handled by _score_and_save now, but ensuring explicit list in target_thought)
            if p_thought.id not in target_thought.children_ids:
        ...
        fusion_thought = self.fusion.fuse_thoughts(
            ...
            model_tier=DEFAULT_MODEL_TIER  # [CHEAP/FAST] logic applied here
        )
