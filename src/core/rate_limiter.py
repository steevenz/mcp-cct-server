...
def rate_limited(
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def async_wrapper(*args, **kwargs):
            # Extract client key
            if key_func:
                client_key = normalize_client_key(key_func(*args, **kwargs))
            else:
                # Default: try session_id, then problem_statement, then first arg, then unknown
                client_key = normalize_client_key(
                    kwargs.get('session_id') or
                    kwargs.get('problem_statement') or
